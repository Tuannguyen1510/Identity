...
namespace Test.Controllers
{
    ...
    public class UsersController : Controller
    {
        ...
        public async Task<IActionResult> Add(AddUserViewModel model)
        {
            ...
            {
                ModelState.AddModelError("Email", "Email is already exits");
                return View(model);
            }
            if (await _userManager.FindByEmailAsync(model.UserName) != null)
            {
                ModelState.AddModelError("UserName", "UserName is already exits");
                return View(model);
            }
          ...
          //  await _userManager.AddToRolesAsync(user,model.Roles.Where(r => r.IsSelected == true).Select(r => r.RoleName));
